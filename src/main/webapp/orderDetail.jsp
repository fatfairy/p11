	...
	<table cellpadding="0" cellspacing="0" align="center" width="100%" class="table table-striped table-bordered table-hover">
		...
		<tr>
			...
			<td colspan="3">
				<table align="center" cellpadding="0" cellspacing="0" width="100%"  class="table table-striped table-bordered table-hover">
					...
					<c:forEach items="${od.productVOS}" var="item" varStatus="i">
						<tr align="center">
							<th>${i.count}</th>
							<th>
								<img src="${item.pImage}" width="50px" height="50px">
							</th>
							<th>${item.pName}</th>
							<th>${item.pState}</th>
							<th>${item.pTime}</th>
		...
		<tr>
			<td align="right" colspan="4" style="margin-right: 40px;">
				<a href="getOrderList" class="btn btn-danger btn-sm">返回订单列表</a>
				&nbsp;&nbsp;
				<c:if test="${od.oState eq 1 }">
					<button type="button" onclick="pay('${od.oId}','${od.oCount}')" class="btn btn-warning btn-sm">支付宝支付</button>
						&nbsp;&nbsp;
